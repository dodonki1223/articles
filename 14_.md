...
**Nested Loops Join**、**Hash Join**、**Sort Merge Join** 以外の結合アルゴリズムについて解説します

## SEMI JOIN（準結合）

**EXISTS** 、**IN** を使用した時に使われるアルゴリズムです

### 特徴

- 結果は必ず **外部表のデータ** から選択される
- 内部表を検索し最初の一致が見つかった時点で **残りの行の検索が打ち切られる** ため、**通常の結合よりパフォーマンスが良い**
- EXISTSとINの結果は **同じ実行計画が選択** される

### 使用すべき時

- **結合先のデータが必要ない時（存在チェックだけよいとか）** はEXISTSとINを使用するのが良さそう。ただし **ちゃんと実行計画を見て確認すること** が大事

### その他

- こちらのサイト：[INとEXISTSはどちらが速いのか？ | サイクル＆オラクル](http://onefact.jp/wp/2018/08/26/semijoin/) に詳しく書かれていて参考になりました
- [Oracle Database SQLチューニングガイド - 結合 - セミ結合](https://docs.oracle.com/cd/E57425_01/121/TGSQL/tgsql_join.htm#GUID-816CED08-10A7-4B39-9790-E68996782847)

## ANTI JOIN（反結合）

**NOT EXISTS** 、**NOT IN** を使用した時に使われるアルゴリズムです

### 特徴

- 結果は必ず **外部表のデータ** から選択される
- 内部表を検索し最初の一致が見つかった時点で **残りの行の検索が打ち切られ** 、一致が見つからなかった行を返すため通常の **結合よりパフォーマンスが良い**
- EXISTS と IN の結果は同じ実行計画が選択されるが、 NOT EXISTSとNOT INは **同じ実行計画が選択されない**

### なぜ NOT EXISTS と NOT IN は同じ実行計画が選択されないのか？

結論から言うと **NULL** により結果が変わってきます

以下のSQLをMySQLで実行すると１行もレコードが返されることはありません  
これは **NULL値は他の値と比較することができない** ためすべての行で不一致扱いになるためです

```sql
-- NOT IN を使用したSQL
SELECT *
  FROM(
                     SELECT NULL     AS hoge, 'キリン'   AS fuga
           UNION ALL SELECT 'あああ' AS hoge, 'ごりら'   AS fuga
           UNION ALL SELECT NULL     AS hoge, 'かば'     AS fuga
           UNION ALL SELECT 'いいい' AS hoge, 'らいおん' AS fuga
           UNION ALL SELECT 'ううう' AS hoge, 'しまうま' AS fuga
      ) AS null_test
 WHERE hoge NOT IN ( 'あああ', NULL)
;
```

しかし以下のSQLをMySQLで実行されるとレコードは hogeカラムが`あああ`以外のレコードは返されます
**NULLや判定が不可能な行に関しては無視される** ため `あああ` 以外のレコードが返されることにある
...
WHERE NOT EXISTS (
                      SELECT *
                        FROM (
                                            SELECT NULL     AS hoge
                                  UNION ALL SELECT 'あああ' AS hoge
                             ) AS not_exists
                       WHERE null_test.hoge = not_exists.hoge
                 )
;
```

**EXISTSとINは機能的には同じ** だが、**NOT EXISTSとNOT IN は機能は同じでない**ため使用する時は注意すること

### その他

- [Oracle Database SQLチューニングガイド - 結合 - アンチ結合](https://docs.oracle.com/cd/E57425_01/121/TGSQL/tgsql_join.htm#GUID-084D65F8-0517-4B85-960F-F1CDEE69C693)
